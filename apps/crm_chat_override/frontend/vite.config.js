...
export default defineConfig(async ({ mode }) => {
  ...
  config.plugins.unshift(
    frappeui({
      ...
      jinjaBootData: true,
      buildConfig: {
        outDir: path.resolve(__dirname, '../crm_chat_override/public/frontend'),
        
        // This is the HTML file Frappe uses to load the app
        indexHtmlPath: path.resolve(__dirname, '../crm_chat_override/www/crm.html'),
        // ───────────────────────────────────────────────────────────────────
        emptyOutDir: true,
        sourcemap: false,
      },
...
function getAliases(config) {
  return {
    ...
    'frappe-ui/tailwind': path.resolve(
      __dirname, '../frappe-ui/tailwind/preset.js',
    ),
    'frappe-ui/style.css': path.resolve(
      __dirname, '../frappe-ui/src/style.css',
    ),
